...
import { DealerInspectionPanel } from "@/components/transaction/dealer-inspection-panel";
import { DealerShippingPanel } from "@/components/transaction/dealer-shipping-panel";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { formatAed, formatDate } from "@/lib/utils";
import { TRANSACTION_STATUS_CONFIG } from "@/lib/constants";
import type { Profile, DealerProfile, TransactionWithDetails } from "@/types";
...
export default async function DealerTransactionDetailPage({ params }: PageProps) {
  ...
  return (
    <DashboardShell navItems={NAV_ITEMS} profile={profile} title="Transaction Details">
      <div className="space-y-6">
        ...
        {tx.status === "inspection_pending" && (
          <DealerInspectionPanel transactionId={tx.id} />
        )}
        {tx.status === "shipping_pending" && (
          <DealerShippingPanel transactionId={tx.id} />
        )}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {tx.buyer?.country && (
                <p className="text-muted-foreground">Country: {tx.buyer.country}</p>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        {tx.dispute && Array.isArray(tx.dispute) && tx.dispute.length > 0 && (
          <Card className="border-red-500/20 bg-red-500/5">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm text-red-400">Active Dispute</CardTitle>
            </CardHeader>
            <CardContent className="text-sm space-y-2">
              <p className="capitalize text-muted-foreground">
                Reason: {tx.dispute[0].reason}
              </p>
              {tx.dispute[0].description && (
                <p className="text-muted-foreground">{tx.dispute[0].description}</p>
              )}
              <p className="text-xs text-muted-foreground">
                Filed: {formatDate(tx.dispute[0].created_at)}
              </p>
            </CardContent>
          </Card>
        )}
